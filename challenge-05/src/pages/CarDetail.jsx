...
import {fectCarDetails } from '../redux/ActionCar'
...
export default function CarDetail() {
  ...
  const { id } = useParams()
  useEffect(() => {dispatch(fectCarDetails(id))})
      ...
      <div className="flex flex-col lg:flex-row justify-between lg:items-start lg:w-9/12 mx-7 lg:mx-auto mt-10">
        <div className="flex flex-col mb-7 mr-7 w-full">
          ...
          <div className="ml-auto">
            <Link to={`/detail-pembayaran/${Cars.id}`}>
        ...
        <aside className="p-6 border-2 border-bgGrey rounded-lg h-auto lg:w-1/2">
          <img className="mx-auto" src={Cars.image} alt="Car" />
          ...
          <div className="flex justify-between">
            <p>Total</p>
            <p className="text-lg font-bold">Rp. {Cars.price}</p>
          </div>
          <Link
            to={`/detail-pembayaran/${Cars.id}`}
